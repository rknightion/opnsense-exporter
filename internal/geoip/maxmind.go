...
//   - If-Modified-Since against the previous Last-Modified returns 304, which MaxMind
//     documents as not counting against the daily download limit.
//   - Bad credentials are 401; an unknown edition is 404.
//   - The download 302s to Cloudflare R2 with a presigned URL. net/http follows it and
//     deliberately does NOT forward the Authorization header across hosts, which is
//     both required and correct — re-sending Basic auth would hand the license key to
//     a third party.
const DefaultEndpoint = "https://download.maxmind.com/geoip/databases"

// defaultMaxBytes caps the decompressed size of an archive member. The endpoint is
// configurable, so a hostile or corrupt archive must not be able to fill the disk;
...
//
// Every failure mode leaves whatever is already installed untouched: the archive is
// verified against its published SHA-256 before anything is extracted, the database is
// written to a temporary file and renamed into place, and a failed fetch simply
// returns an error. The caller (see updater.go) logs it and tries again on the next
// tick.
type Downloader struct {
	...
	Dir string
	// Timeout bounds one edition's fetch end to end. Zero means no timeout beyond the
	// caller's context.
	Timeout time.Duration
	// MaxBytes caps a decompressed archive member; zero selects defaultMaxBytes.
	MaxBytes int64
	// Client is the HTTP client to use; nil builds one. It MUST follow redirects (the
	// default client does) — the download 302s to R2.
	Client *http.Client
}
...
type DownloadResult struct {
	Edition string
	// Path is where the database is installed, whether or not this run updated it (a
	// 304 leaves the previously installed file in place).
	Path string
	// Updated is false when the server answered 304 Not Modified.
	Updated bool
	// BuildTime is the archive's Last-Modified, i.e. MaxMind's build date. Zero on a
	// 304 (the server sends no Last-Modified with one).
	BuildTime time.Time
}

// DatabasePath is where an edition's database is installed. The options layer uses it
// to default --geoip.country-database / --geoip.asn-database when only the downloader
// is configured, so it must agree with Fetch.
...
// filename. The name is interpolated into both, so this runs before a request is made
// rather than sanitizing afterwards into something the operator did not ask for.
...
// Fetch downloads, verifies and installs one edition, returning what it did. A 304
// Not Modified is a success with Updated=false.
func (d *Downloader) Fetch(ctx context.Context, edition string) (DownloadResult, error) {
	...
	// 0o750, not 0o755: the databases are this process's state, and nothing else on
	// the host needs to read them.
	if err := os.MkdirAll(d.Dir, 0o750); err != nil {
		return res, fmt.Errorf("create geoip database directory %s: %w", d.Dir, err)
	}

	// The installed file's mtime IS the conditional-request state: Fetch sets it to
	// the server's Last-Modified after a successful install, so the next run can ask
	// "anything newer than this?" exactly, with no sidecar metadata file to keep in
	// sync (or to go stale against a hand-replaced database).
	var since time.Time
	...
	if sum != want {
		return res, fmt.Errorf("geoip database %s failed its checksum: archive is %s, MaxMind published %s",
			edition, sum, want)
	}
...
func (d *Downloader) get(ctx context.Context, edition, suffix string, since time.Time) (
	body io.ReadCloser, lastModified time.Time, notModified bool, err error,
) {
	endpoint := d.Endpoint
	if endpoint == "" {
	...
	// Basic auth, never a query parameter: a credential in the URL lands in every
	...
	default:
		...
		// are harmless, but a misconfigured endpoint could echo the request back, and
...
// spool streams the archive to a temporary file, hashing as it goes, and returns the
// temp path and the hex digest. Hashing the bytes on the way to disk means the
// archive is read once and never held in memory.
func (d *Downloader) spool(body io.Reader) (string, string, error) {
	...
	if _, err := io.Copy(io.MultiWriter(f, h), io.LimitReader(body, d.maxBytes())); err != nil {
		// Best-effort cleanup: the download already failed, and a leftover temp file
		// in the state directory is not worth masking that error with.
		_ = os.Remove(f.Name())
...
func (d *Downloader) install(ctx context.Context, archive, dest, edition string, lastModified time.Time) error {
	...
	if !lastModified.IsZero() {
		// Not fatal: the database is installed and usable. The only loss is that the
		// next conditional request re-downloads once.
		_ = os.Chtimes(dest, lastModified, lastModified)
	}
	return nil
}

// extract writes the archive's single .mmdb member to a temporary file beside dest
// and returns its path.
//
// The member's NAME is never used to build a path — the destination is always
// <Dir>/<Edition>.mmdb, chosen by us — so archive traversal ("../../etc/...") is
...
func (d *Downloader) extract(ctx context.Context, archive, dest, edition string) (string, error) {
	...
	for {
		...
		if h.Typeflag != tar.TypeReg {
			return "", fmt.Errorf("geoip archive for %s holds a non-regular .mmdb member (type %q)",
				edition, h.Typeflag)
		}
		if h.Size > d.maxBytes() {
			return "", fmt.Errorf("geoip database in the %s archive is %d bytes, above the %d-byte limit",
				edition, h.Size, d.maxBytes())
		}
