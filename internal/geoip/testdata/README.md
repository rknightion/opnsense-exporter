...
`test-data/`. That repository is dual-licensed MIT / Apache-2.0, which permits
redistribution; the databases hold synthetic records, not a licensed GeoLite2 extract.

**No real GeoLite2 or GeoIP2 database is committed to this repository** — they are tens of
megabytes each, rebuilt twice a week, and covered by the GeoLite End User License
Agreement. Operators supply their own, or let `--geoip.download.enabled` fetch them.
