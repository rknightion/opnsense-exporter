...
// resolved from the flag values. Endpoints absent from the map — which is all of
// them but these — are never cached.
func CacheTTLs() EndpointCacheTTLs {
	return BodyCacheTTLs(*cacheTTL, *firmwareCacheTTL)
}

// BodyCacheTTLs is the pure form of CacheTTLs, taking the two TTL knobs as
// arguments instead of reading the parsed flags. It exists so the cached-endpoint
// set can be asserted against the poll tiers without a kingpin parse — see
// TestBodyCachedEndpointsPollSlowerThanTheirTTL, which is what stops a body TTL
// being attached to an endpoint whose collector polls faster than the cache.
func BodyCacheTTLs(cacheTTL, firmwareCacheTTL time.Duration) EndpointCacheTTLs {
...
// so a firewall without the plugin is not asked for it on every poll. 0 disables
...
//
// Unlike a body TTL this is independent of poll cadence: a 404 says the route does
// not exist (the plugin is not installed), which changes only by admin action, so it
// is safe to remember for a fast-polling collector as well as a slow one.
