...
func TestGracefulShutdownDrainsInFlightRequest(t *testing.T) {
	...
	var pollStopped, cpuStreamStopped, logsStopped, otlpStopped, profStopped, annotationsStopped bool
	gracefulShutdown(ts.Config, syscall.SIGINT,
		func() { pollStopped = true },
		func() { cpuStreamStopped = true },
	...
	if !pollStopped || !cpuStreamStopped || !logsStopped || !otlpStopped || !profStopped || !annotationsStopped {
		t.Errorf("stop hooks not run: poll=%v cpu_stream=%v logs=%v otlp=%v prof=%v annotations=%v",
			pollStopped, cpuStreamStopped, logsStopped, otlpStopped, profStopped, annotationsStopped)
	}
