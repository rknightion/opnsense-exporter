...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 The promoted stream-label set observed in Loki matches the documented table in docs/log-shipping.md, and any divergence is written up rather than silently accepted
- [x] #2 OPN-0038 domain enrichment is confirmed present as structured metadata and confirmed ABSENT from the stream-label set, per its frozen contract
- [x] #3 Config revision and config snapshot bodies are confirmed redacted in the delivered record, not merely in the unit test
- [x] #4 No credential value appears in any commit, tracker entry, log or report; the run is identifiable by its instance label so the data can be pruned
- [x] #5 Every log source the exporter can produce on the testbed without a firewall mutation is observed arriving in the m7kni stack, identified by its opnsense.source value, with the query used and a redacted result recorded (configchange, configstate with its three families, exporter, syslog, unbound, zenarmor, plus merged and netflow from production). routingchange and a specifically unparsed syslog record are recorded as never observed because producing them needs a testbed mutation, which is refused (owner decision 2026-09-05).
<!-- AC:END -->
...
Attended resolution 2026-09-05 22:45Z (after wave 8). Read-only Loki query of {service_name=opnsense2otel,opnsense_source=configchange} over the successor revision timestamp (1788507398) plus/minus 1h, with categorize-labels: TWO records present, under delivery-proof-33982574173 and delivery-proof-33984106411 (the wave 7 runs), each 1472 bytes / 43 lines, byte-identical to each other and the same size wave 8 emitted, structured metadata carrying revision and user. So configchange DELIVERY WORKS and always did; every run's own query missed its own record. Cause, confirmed against Loki docs (querier query_ingesters_within, default 3h): a query whose range is older than the lookback is not sent to ingesters and reads only the store, and a record stamped 35h in the past sits in an ingester chunk until that chunk flushes (max_chunk_age / idle timeout), so it is invisible for up to roughly two hours after shipping. Wave 8's own record (delivery-proof-33995443013, shipped 22:17Z) was still absent at 22:45Z, consistent with that. Verifier built from HEAD 22338a12 (expanded SensitiveConfigKey) over the two delivered configchange bodies: configchange_bodies=2, configchange_sensitive_elements=6, configchange_bodies_redacted=true, i.e. six sensitive elements were present in the real diff and every one arrived as the redaction marker. Structural shape only: 4 hunk headers, 26 changed lines, 3 marker lines; no body content retained. Combined with wave 8's 693 configstate bodies at the same vocabulary, AC4 is satisfied: config revision and snapshot bodies are redacted on the wire. Assertion 1 (arrival) is YES on the wave 7 instances. AC1 remainder: opnsense_source values observed in m7kni over 30 days are configchange, configstate, exporter, merged, netflow, syslog, unbound, zenarmor; routingchange has never been observed (no route change occurs on the testbed without a mutation, which is refused) and an unparsed-syslog record has not been specifically observed; ids and crowdsec are outside this task. Decision on AC1 is Rob's.
<!-- SECTION:NOTES:END -->

## Final Summary

<!-- SECTION:FINAL_SUMMARY:BEGIN -->
Closed 2026-09-05 after wave 8 plus an attended read-only Loki check. Proven: end-to-end OTLP transport and the documented seven-key stream-label set on production data; dst_domain as structured metadata and never a stream label; all three configstate families arriving under a proof instance (wave 7, wave 8); configchange arriving (the wave 7 records for instances delivery-proof-33982574173 and -33984106411 are in m7kni Loki at the successor revision timestamp, 1472 bytes / 43 lines each); and on-wire redaction of both body kinds under the expanded SensitiveConfigKey vocabulary (verifier over the delivered configchange bodies: 2 bodies, 6 sensitive elements, all redacted; wave 8 run 33995443013: 693 configstate bodies, 0 sensitive keys). Why three runs each missed their own configchange record: the Loki querier does not query ingesters beyond query_ingesters_within (3h), and a record stamped with a revision time roughly 35h old lives in an unflushed ingester chunk for up to about two hours; wave 8 also proved the exporter side (Poll emitted 1 record, logs_shipped_total 1, every drop reason 0, partialSuccess handled). Docs and harness follow-up is OPN-0099. AC1 was narrowed by owner decision: routingchange and a specifically unparsed syslog record were never observed because producing them needs a testbed mutation, which is refused. Release 4.2.0 (PR 679) was held on this task and merged on the double yes at 2026-09-05T22:55:54Z as e41683f7. Nothing on the testbed was created, edited or deleted across waves 6 to 8; no credential value reached any artifact.
