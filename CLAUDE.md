...
- **`internal/collector/`** — Prometheus collector implementations. `collector.go` holds the top-level `Collector` struct. An internal **poll scheduler** (`scheduler.go`, #336) runs each of the 61 sub-collectors' `Update()` on its own interval — its data-volatility tier (`interval_tiers.go`: fast 15s / medium 60s / slow 5m / cold 15m) — into an in-memory **snapshot** (`snapshot.go`). Serving `/metrics` (`ScrapeView`) and the OTLP bridge both **replay that snapshot**: the request path makes no live API call. Each sub-collector (one file per subsystem) implements `CollectorInstance` with `Name()`, `Register()`, `Describe()`, and `Update()`. **Sub-collectors register themselves via `init()` functions** appending to the global `collectorInstances` slice — adding a new collector requires only creating the file with an `init()` function.
