...
!!! warning "Minimum Grafana version: 13+"
    The dashboard uses the v2 dynamic schema (`dashboard.grafana.app/v2`) with `TabsLayout` and `conditionalRendering`, which require **Grafana 13 or later**. On Grafana 11 or 12, import [`dashboard-compat.json`](#grafana-11-and-12-use-the-compatibility-dashboard) instead.
...
### Grafana 11 and 12: use the compatibility dashboard

`grafana/dashboard.json` is schema v2 and Grafana 11/12 cannot read it. **Neither version tells you that clearly**, which is worth knowing before you spend time on it (both verified against pinned `grafana/grafana` containers, 2026-07-27):

| Grafana | Importing `dashboard.json` |
| --- | --- |
| 11.5.0 | `POST /api/dashboards/db` → **HTTP 400**, `Dashboard title cannot be empty` — v2 keeps the title under `spec`, so the error names a symptom rather than the cause |
| 12.4.0 | `POST /api/dashboards/db` → **HTTP 200**, and the dashboard then renders with **0 panels and 0 variables**. The v2 body is stored verbatim and ignored |
| 12.4.0 | `POST /apis/dashboard.grafana.app/...` (the CRD/provisioning route) → **HTTP 400**, `no kind "Dashboard" is registered for version "dashboard.grafana.app/v2"` |

Import **`grafana/dashboard-compat.json`** instead. It is generated from the same source as the canonical dashboard — same panels, queries, units, thresholds and variables — and CI imports it into Grafana 11.5.0 and 12.4.0 on every commit and reads it back, so the support claim is tested rather than asserted.

What it deliberately does **not** carry:

- **No tabs.** Classic schema has none, so each tab's rows are flattened in order with the tab name on the first row of each group.
- **No conditional rendering, so no plugin-gated content.** This is the one that changes what you see: on Grafana 13 the dashboard hides a tab whose metrics your box does not emit. Classic cannot, so a gated tab would be permanently visible and permanently empty — indistinguishable from a broken exporter. The compatibility build therefore carries only the **187 panels that are unconditional on every OPNsense box** (Overview, System & Resources, Services/Cron/DynDNS, Certificates, Interfaces, Gateways & WAN, Routing & Neighbors, Protocol Stats, Firewall & PF, Diagnostics) out of the canonical 738.

If you run a plugin whose tab is missing here, the fix is Grafana 13, not a hand-edit — an edited copy stops tracking the generated one and silently rots. The compatibility artifact is regenerated by `make compat`, and `make grafana-check` fails if it is stale.
