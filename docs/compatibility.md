...
One binary handles every supported payload shape at once. There are no compatibility flags,
no version-detection switch, and nothing to configure — the API client resolves payload
differences **by shape**, reading whichever field a given firewall actually sends. Point the
same image at a 25.7 box and a 26.1 box and both scrape correctly.

When a release drops out of the support window, the shims that carried its payload shape are
pruned. That pruning is a normal release change, not a breaking one, because by then no
supported firewall sends the old shape.

## Version-dependent data availability

Some data is gone from the OPNsense API itself. Where upstream stopped serving a field, the
...
source-tracking statistics (both 26.1.11). Nothing to do about these — they are listed here only
so a payload diff against an older firewall does not read as a regression.
