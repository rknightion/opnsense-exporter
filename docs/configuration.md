...
| `--geoip.enabled` | `OPNSENSE_EXPORTER_GEOIP_ENABLED` | `false` | Enable local GeoIP enrichment of flow records from MaxMind .mmdb files on disk. Adds country/continent/city/ASN attributes to flow LOGS for external addresses, so geo no longer depends on whether Zenarmor happened to see the connection. Purely local: no lookup ever touches the network. Off by default because it needs a database the exporter does not ship. |
