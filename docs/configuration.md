...
| `--annotations.lookback` | `OPNSENSE_EXPORTER_ANNOTATIONS_LOOKBACK` | `24h` | How old an event may be and still be worth annotating, and how far back the startup reconciliation looks for annotations this exporter already wrote. Keeps a fresh deployment from annotating a reboot that happened months ago. |
| `--annotations.max-per-cycle` | `OPNSENSE_EXPORTER_ANNOTATIONS_MAX_PER_CYCLE` | `20` | Maximum annotations written per check. A guard against one bad reading writing hundreds of annotations, not a rate limit to tune. |
