...
This package contains the top-level `Collector` struct and all 82 sub-collectors. Each sub-collector lives in its own file and implements the `CollectorInstance` interface:
...
Goroutine-leak profiling is generally available in Go 1.27. The exporter still checks runtime availability before registering the profile, so an unsupported runtime silently omits that one type; everything else is unchanged. It periodically runs a short stop-the-world analysis (at each upload) to surface goroutines that are blocked forever - cheap for an exporter's modest goroutine count.

**`goroutine_leak` absent from a Grafana Cloud profile-type listing is expected on a healthy process, not a delivery fault.** The vendored pyroscope-go SDK (`vendor/github.com/grafana/pyroscope-go/session.go`) uploads this profile type on every upload cycle unconditionally - it never skips the upload when the profile has zero samples, unlike, say, choosing not to call the endpoint at all. Verified 2026-08-04 with a Go 1.26 experiment build pointed at a local HTTP sink: with no leaked goroutines, every cycle POSTs a valid pprof profile with an empty sample list (`go tool pprof -raw` shows `Samples:` with nothing beneath it, ~192 bytes decompressed); the moment a goroutine was deliberately leaked (blocked forever on an unbuffered channel), the very next upload cycle carried exactly one sample with the correct blocking stack trace, and every subsequent cycle kept reporting it for as long as the leak persisted. So the pipeline works end to end - detection, serialization, and upload are all confirmed functioning - and an idle window with nothing leaked genuinely has nothing to show. This is why a healthy fleet with no goroutine leaks can go days without `goroutine_leak` appearing in a query, while `process_cpu` and `goroutines` (which always carry live data) do not.
