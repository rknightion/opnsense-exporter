...
| --- | --- | --- |
| `api/diagnostics/traffic/top/<interfaces>` | A two-second iftop sample with a ten-second timeout per interface, capped upstream at 200 hosts per interface since OPNsense 26.7.3. The opt-in `pftop` collector polls it on the slow tier, ranks a deterministic global top 100, carries overflow accounting, and keeps a five-minute bounded identity inventory. It is a sampled fallback, not complete traffic accounting; prefer the NetFlow receiver and do not treat the two sources as interchangeable. | `TrafficController::topAction()` calls `interface show top`; `traffic_top.py` runs one `iftop -nNb -i <if> -s 2 -t` process per interface and sorts `rate_bits` before `[:200]`. The former no-cap statement was disproved against released 26.7.3 source. |
