...
update verdict, pending package versions, certificate-expiry roll-up, API-key owners and
a bounded `listening_sockets` aggregate. The listener count is derived from OPNsense's
`listen-queue-sizes` marker for Internet and UNIX-domain accept-queue sockets; it does not
count ordinary UDP sockets or established connections. A content hash suppresses unchanged
batches; the firewall and device families repeat after a six-hour heartbeat, while security
posture repeats weekly. Records share an opaque `snapshot.id` and
