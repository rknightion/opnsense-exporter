...
- **OTLP metrics export** — optional OpenTelemetry push (enable with `--otlp.enabled`) that exports the **exact same metrics** served at `/metrics` to an OTLP endpoint (gRPC or HTTP/protobuf), with identical names, labels and values. It uses the OpenTelemetry `contrib/bridges/prometheus` producer over the existing Prometheus registry, so there is zero per-collector duplication and the pull endpoint is unaffected. Honours the standard `OTEL_EXPORTER_OTLP_*` / `OTEL_RESOURCE_ATTRIBUTES` env vars, always exports cumulative temporality (inherent to the Prometheus source and required by Grafana Cloud/Mimir), and includes a Grafana Cloud instance-id+token shortcut. See [configuration](docs/configuration.md#otlp-metrics-export).
