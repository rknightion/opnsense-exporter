...
| Path | What it is |
|------|------------|
| `dashboard.json` | The dashboard — a single **Grafana v2 dynamic dashboard** (`dashboard.grafana.app/v2`) with 36 tabs that auto-show/hide based on which metrics your exporter emits. |
| `dashboard.json` | The dashboard — a single **Grafana v2 dynamic dashboard** (`dashboard.grafana.app/v2`) with 36 tabs that auto-show/hide based on which metrics your exporter emits. |
| `dashboard.json` | The dashboard — a single **Grafana v2 dynamic dashboard** (`dashboard.grafana.app/v2`) with 36 tabs that auto-show/hide based on which metrics your exporter emits. |
...
One dashboard, 36 tabs (generated list, do not hand-edit):

<!-- docgen:begin:dashboard-tabs -->
Overview, System & Resources, Interfaces, Firewall & PF, Aliases, Gateways & WAN, DNS — Unbound, DHCP, VPN, Tailscale, NetBird, Routing & Neighbors, Protocol Stats, NTP, Certificates, ClamAV, Services, Cron & DynDNS, Syslog, Q-Feeds, NetFlow, CARP / HA, HAProxy, Relayd, Nginx, FRR Routing, Monit, CrowdSec, IDS/IPS, UPS, Captive Portal, Traffic Shaper, HA Sync, Chrony, Tor, Siproxd, Diagnostics
