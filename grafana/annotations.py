...
ANNOTATIONS: list = [
    ...
    Annotation(
        ...
        why="Explains a tunnel's traffic series starting or stopping. Default-ON by "
            ...
            "#596 added two more, verified against the emitters): vpn.event is set by "
            "openvpn and charon per-session/per-SA, and by wireguard and tailscaled "
            "as SERVICE-level edges — a WireGuard instance starting, stopping or "
            "being switched up/down by CARP, and this node's own Tailscale ipn state "
            "entering or leaving Running. Two consequences worth knowing before "
            "reading a marker: on WireGuard and Tailscale the marker is the tunnel "
            "INSTANCE or the whole node, never one peer, because nothing under those "
            "app-names names a peer (the per-peer WireGuard handshake lines arrive as "
            "kernel, behind a debug flag, and repeat every ~2 minutes for a HEALTHY "
            "peer, so they are deliberately not events); and on a stock box "
            ...
            "service-stop teardown line arrives until an operator enables it. "
            "STILL UNCOVERED: netbird. Its daemon publishes under app-name "
            "/usr/local/bin/netbird, not netbird (Go's log/syslog substitutes argv[0] "
            "for netbird's empty tag), and its peer open/close lines are driven by "
            "the lazy-connection idle timer rather than by tunnel health — see the "
            "comment in internal/logship/syslog/registry.go. The subsystem regex "
            "stays deliberately wide, because narrowing it to the covered programs "
            "would make that remaining gap invisible in the query as well.",
    ),
    Annotation(
