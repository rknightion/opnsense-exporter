...
ANNOTATIONS: list = [
    ...
    Annotation(
        ...
        why="Explains a tunnel's traffic series starting or stopping. Default-ON by "
            ...
            "COVERAGE (#591 item 6a found this layer covering only two backends; "
            "#596 added two more and #601 the fifth, each verified against the "
            "emitter): vpn.event is set by openvpn and charon per-session/per-SA, and "
            "by wireguard, tailscaled and netbird as SERVICE-level edges — a WireGuard "
            "instance starting, stopping or being switched up/down by CARP, this "
            "node's own Tailscale ipn state entering or leaving Running, and the "
            "NetBird engine coming up with an overlay address or its connect loop "
            "finishing teardown. ALL FIVE BACKENDS ARE NOW COVERED, so a silent layer "
            "means no tunnel flaps rather than no parser. Three consequences worth "
            "knowing before reading a marker: on WireGuard, Tailscale and NetBird the "
            "marker is the tunnel INSTANCE or the whole node, never one peer, because "
            "nothing usable under those app-names names a peer (the per-peer WireGuard "
            "handshake lines arrive as kernel, behind a debug flag, and repeat every "
            "~2 minutes for a HEALTHY peer; NetBird's per-peer open/close lines are "
            "driven by a 15-minute lazy-connection idle timer gated on a server-side "
            "feature flag — so both are deliberately not events); on a stock box "
            "tailscaled's own log is not routed to syslog at all (the FreeBSD rc "
            "script's tailscaled_syslog_output_enable defaults to NO), so only its "
            "service-stop teardown line arrives until an operator enables it; and "
            "NetBird's markers are keyed on app-name /usr/local/bin/netbird, which is "
            "argv[0] rather than a program name (Go's log/syslog substitutes argv[0] "
            "for netbird's empty tag) — captured live, see "
            "internal/logship/syslog/netbird.go. The subsystem regex stays "
            "deliberately wide. It is not narrowed to the covered programs even now "
            "that all five are covered: the wide form is what makes a future backend's "
            "records visible as vpn traffic with no vpn_event, which is how this whole "
            "class of gap was found in the first place.",
    ),
    Annotation(
